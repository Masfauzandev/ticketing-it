<x-layouts.app>
    ...
    @section('page-subtitle', 'Manajemen Data Divisi Sistem')
    ...
    <div class="mx-auto max-w-6xl" x-data="{ 
        ...
        <div class="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <h2 class="text-xl font-bold th-text">Daftar Divisi</h2>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="border-b th-border" style="background-color: var(--t-bg-input);">
                        <tr>
                            <th class="whitespace-nowrap px-6 py-4 font-semibold th-text">Nama Divisi</th>
                    ...
                    <tbody class="divide-y th-border">
                        @forelse($divisions as $division)
                            <tr class="transition-colors hover:th-bg-hover">
                                <td class="px-6 py-4 font-medium th-text">{{ $division->name }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <button
                                            @click="editingDivision = {{ json_encode($division) }}; editModalOpen = true"
                                        ...
                                        <button
                                            @click="deletingDivision = {{ json_encode($division) }}; deleteModalOpen = true"
        ...
        <div x-show="addModalOpen"
            ...
            <div class="w-full max-w-md rounded-2xl border th-border p-6 shadow-2xl" @click.away="addModalOpen = false"
                ...
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-lg font-bold th-text">Tambah Divisi</h3>
                ...
                <form action="{{ route('admin.divisions.store') }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label class="mb-1 block text-sm font-medium th-text">Nama Divisi</label>
        ...
        <div x-show="editModalOpen"
            ...
            <div class="w-full max-w-md rounded-2xl border th-border p-6 shadow-2xl" @click.away="editModalOpen = false"
                style="background-color: var(--t-bg-card);">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-lg font-bold th-text">Edit Divisi</h3>
                ...
                <form :action="editingDivision ? '{{ url('admin/divisions') }}/' + editingDivision.id : ''"
                    method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label class="mb-1 block text-sm font-medium th-text">Nama Divisi</label>
                        <input type="text" name="name" :value="editingDivision ? editingDivision.name : ''" required
                            class="w-full rounded-xl border th-border px-4 py-2 text-sm th-text focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
                            style="background-color: var(--t-bg-input);">
                    </div>
                    <div class="mb-6 flex items-center gap-2">
                        <input type="checkbox" name="is_active" id="edit_is_active" value="1"
                            :checked="editingDivision && editingDivision.is_active"
                            class="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500">
        ...
        <div x-show="deleteModalOpen"
            ...
            <div class="w-full max-w-sm text-center rounded-2xl border th-border p-6 shadow-2xl"
                ...
                <h3 class="mb-2 text-xl font-bold th-text">Hapus Divisi?</h3>
                <p class="mb-6 text-sm th-text-muted">
                    Apakah Anda yakin ingin menghapus divisi <strong class="th-text"
                        x-text="deletingDivision ? deletingDivision.name : ''"></strong>?<br>Tindakan ini tidak dapat
                    dibatalkan.
                </p>
                <form :action="deletingDivision ? '{{ url('admin/divisions') }}/' + deletingDivision.id : ''"
